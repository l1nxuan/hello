...
This is an embedded firmware project targeting the **STM32F429ZI** microcontroller (ARM Cortex-M4 with FPU) running at 168 MHz. It features a GUI application using the ThreadX RTOS and LVGL graphics library. The project is designed for the Armfly V6 development board.
...
|-----------|---------|---------------|
| ThreadX | Real-time operating system | Azure RTOS |
...
│   ├── demo_waveform.c           # Waveform generation demo
│   ├── demo_waveform.h
...
│   │   ├── demo.eez-project      # EEZ Studio project file
│   │   └── src/
...
│   │       ├── vars.h
│   │       └── structs.h
│   └── sample/                   # Sample/ADC data acquisition thread
│       ├── sample.c              # ADC sampling thread implementation
│       └── sample.h
...
| Sample Thread | 4 KB | 0 | ADC data acquisition and waveform processing |
...
The Sample thread waits on a semaphore (`convcplt_sem`) that is signaled by ADC DMA completion callbacks. Data is shared between the Sample thread and Display thread via the `refresh_events` flag and `chart_1_serie_1_buffer` array.
